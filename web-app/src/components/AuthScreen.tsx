 ...
 * 
 * Handles user login, signup, and OAuth authentication
 */
...
export default function AuthScreen({ onAuthSuccess }: AuthScreenProps) {
  ...
  const handleOAuth = async (provider: 'google' | 'microsoft' | 'apple') => {
    ...
    try {
      ...
      if (provider === 'google') {
        ...
        ({ error } = await authService.signInWithMicrosoft());
      } else {
  ...
  const handleDemo = async () => {
    setIsLoading(true);
    try {
      const { user, error } = await authService.createDemoAccount();
      if (error) throw error;
      if (user) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 sm:p-8 max-w-md w-full">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-red-500 text-white py-2 px-4 rounded-lg hover:bg-red-600 dark:hover:bg-red-700 transition-colors disabled:opacity-50"
          >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 sm:p-8 max-w-md w-full">
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4 mb-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-primary text-white py-2 px-4 rounded-lg hover:bg-blue-600 dark:hover:bg-blue-700 transition-colors disabled:opacity-50"
          >
        ...
        <div className="space-y-3 mb-6">
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 py-2 px-4 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 bg-black dark:bg-gray-900 text-white py-2 px-4 rounded-lg hover:bg-gray-800 dark:hover:bg-gray-700 transition-colors disabled:opacity-50"
          >
